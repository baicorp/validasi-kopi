...
CREATE TABLE `user` (
	...
	FOREIGN KEY (`plant_area_id`) REFERENCES `plant_areas`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `exam_events` (
	...
	`code_group_reguler_id` text NOT NULL,
	`code_group_retake_id` text NOT NULL,
	`created_at` text DEFAULT (CURRENT_TIMESTAMP),
	`updated_at` text DEFAULT (CURRENT_TIMESTAMP),
...
CREATE TABLE `exam_submission_notes` (
	...
	`created_at` text DEFAULT (CURRENT_TIMESTAMP),
	`updated_at` text DEFAULT (CURRENT_TIMESTAMP),
	FOREIGN KEY (`submission_attempt_id`) REFERENCES `submission_attemps`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
